...
MODEL_PATH = f"s3://{S3_BUCKET}/models/youbike_lgbm_model.joblib"
...
model = wr.s3.read_joblib(path=[MODEL_PATH])[0]
...
def lambda_handler(event, context):
    ...
    df_historical = wr.s3.read_parquet(
        ...
        dataset=True,
        partition_filter=lambda x: datetime(int(x["year"]), int(x["month"]), int(x["day"])) >= start_time.date()
    )
